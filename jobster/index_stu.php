...
$job_search=mysqli_real_escape_string($con,$job_search);
...
//all jobs
if($cid != "" ){
    $sql_jobs0="select jid,cid,title,cname,job.location,salary,requirement, job.desc,endTime from job inner join company using(cid) where cid = '$cid';";
}elseif($job_search != ""){
    $sql_jobs0 = "select jid,cid,title,cname,job.location,salary,requirement, job.desc,endTime from job inner join company using(cid) where job.location  like '%".$job_search."%' or job.desc like '%".$job_search."%' or cname like '%".$job_search."%' or title like '%".$job_search."%' or requirement like '%".$job_search."%';";
}else{
    $sql_jobs0="select jid,cid,title,cname,job.location,salary,requirement, job.desc,endTime from job inner join company using(cid);";
}
                        ...
                        <div class="work-progres">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                    for ($i = 0; $i < $count_jobs0; $i++){
                                        if($row=mysqli_fetch_assoc($result_jobs0)){
                                            $cid = $row['cid'];
                                            $cname = $row['cname'];
                                            $title = $row['title'];
                                            $salary = $row['salary'];
