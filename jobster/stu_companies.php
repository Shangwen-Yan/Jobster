<?php
    ...
    //for body
    // followed companies
    $sql_comp0="select cid,cname,location,industry,magnitude from company inner join follow using(cid) where sid = '$sid';";
    $result_comp0= mysqli_query($con,$sql_comp0);
    $count_comp0=mysqli_num_rows($result_comp0);

    //unfollowed companies
    $sql_comp1="select cid,cname,location,industry,magnitude from company where cid not in (select cid from company inner join follow using(cid) where sid = '$sid');";
    $result_comp1= mysqli_query($con,$sql_comp1);
    $count_comp1=mysqli_num_rows($result_comp1);
...
<script type="text/javascript" language="JavaScript">
    $(function() {
        ...
        $(".followc").mouseover(function(){
            $(".followc").css("cursor","pointer");

        });
        $(".unfollowc").mouseover(function(){
            $(".unfollowc").css("cursor","pointer");

        });

        $(".followc").click(function(){
            $.ajax({
                url:'follow_comp.php',
                data:{'cid':$(this).next().html(),'sid':<?php echo $sid?>},
                type :'POST',
                dataType:'JSON',
                success:function(msg){
                    window.location.href ="stu_companies.php";
                },
                error:function(msg){
                }
            });
        ...
        $(".unfollowc").click(function(){
            $.ajax({
                url:'follow_cancel.php',
                data:{'cid':$(this).next().html(),'sid':<?php echo $sid?>},
                type :'POST',
                dataType:'JSON',
                success:function(msg){
        ...
        $(".open").click(function(){
            ...
            $("#comp_detail").html("");
            $.ajax({
                url:'get_comp_detail.php',
                data:{'cid':$(this).next().html()},
                type :'POST',
                dataType:'JSON',
                success:function(msg){
                    $("#comp_detail").prev().html("<a href='stu_cblog.php?cid="+msg.cid+"'>"+msg.cname+"</a>");
                    $("#cid_follow").html(msg.cid);
                    $("#comp_detail").html($("#comp_detail").html()+"<p style=\"text-align: left; \"><b>Location: </b>"+msg.location+"</p>");
                    $("#comp_detail").html($("#comp_detail").html()+"<p style=\"text-align: left; \"><b>Industry: </b>"+msg.industry+"</p>");
                    $("#comp_detail").html($("#comp_detail").html()+"<p style=\"text-align: left; \"><b>Magnitude: </b>"+msg.magnitude+"</p>");
                    $("#comp_detail").html($("#comp_detail").html()+"<p style=\"text-align: left; \"><b>Description: </b>"+msg.cdesc+"</p>");
        ...
        $("#send_form>[type='button']").click(function () {
            window.location.href ="index_stu.php?cid="+$("#cid_follow").html();
                        ...
                        <div class="work-progres">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                        for ($i = 0; $i < $count_comp0; $i++){
                                            if($row=mysqli_fetch_assoc($result_comp0)){
                                                $cid = $row['cid'];
                                                $cname = $row['cname'];
                                                ...
                                                echo "
                                                    <tr>
                                                        ...
                                                        <td><span class=\"label label-success unfollowc\">o</span><span  hidden>".$cid."</span></td>
                                                        <td ><span  id='open' class=\"label label-info open\">open</span><span  hidden>".$cid."</span></td>
                                                    </tr>
                                                ";
                                    ...
                                    ?>
                                    <?php
                                    for ($i = 0; $i < $count_comp1; $i++){
                                        if($row=mysqli_fetch_assoc($result_comp1)){
                                            $cid = $row['cid'];
                                            $cname = $row['cname'];
                                            $industry = $row['industry'];
                                            $magnitude = $row['magnitude'];
                                            echo "
                                                    <tr>
                                                        ...
                                                        <td><span class=\"label label-default followc\">-</span><span  hidden>".$cid."</span></td>
                                                        <td ><span  id='open' class=\"label label-info open\">open</span><span  hidden>".$cid."</span></td>
                                                    </tr>
                                                ";
                        ...
                        <div class="col-md-6 chart-layer1-right" id = "msg_right" style="display: none; width: 40%" >
                            <div class="user-marorm">
                                ...
                                <div class="malorm-bottom">
                                    ...
                                    <span id="cid_follow" hidden></span>
                                    <form id="send_form">
                                        <input type="button" value="all jobs" style="margin-left:250px ">
                                    </form>
