<?php
    ...
    //for body
    //unread applications
    $sql_app0="select sid,sname,title,jid from application inner join job using(jid) inner join student using (sid) where cid='$cid' and status = 0 order by appTime desc;";
    $result_app0= mysqli_query($con,$sql_app0);
    $count_app0=mysqli_num_rows($result_app0);

    //unread applications
    $sql_app1="select sid,sname,title,jid,status from application inner join job using(jid) inner join student using (sid) where cid='$cid' and status != 0 order by appTime desc;";
    $result_app1= mysqli_query($con,$sql_app1);
    $count_app1=mysqli_num_rows($result_app1);
...
<script type="text/javascript" language="JavaScript">
    $(function() {
        ...
        $(".open").click(function(){
            ...
            $("#stu_detail").html("");
            $("#comp_sid").html($(this).next().html());
            $("#comp_jid").html($(this).next().next().html());

            $.ajax({
                url:'app_read.php',
                data:{'jid':$(this).next().next().html(),'sid': $(this).next().html()},
                type:'POST',
                dataType:'JSON',
        ...
        $(".app").mouseover(function(){
            $(".app").css("cursor","pointer");

        });
        $("#accept").click(function(){
            $.ajax({
                url:'app_accept.php',
                data:{'jid':$("#comp_jid").html(),'sid': $("#comp_sid").html()},
                type:'POST',
                dataType:'JSON',
                success: function (msg) {
                    alert("accept successfully!")
                    window.location.href ="comp_application.php";
                },
                error: function (msg) {
                        ...
                        <div class="work-progres">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                        for ($i = 0; $i < $count_app0; $i++){
                                            if($row=mysqli_fetch_assoc($result_app0)){
                                                $sid = $row['sid'];
                                                $sname = $row['sname'];
                                                $title = $row['title'];
                                                $jid = $row['jid'];
                                                echo "
                                                    <tr>
                                                        ...
                                                        <td ><span  id='open' class=\"label label-info open\">open</span><span  hidden>".$sid."</span><span  hidden>".$jid."</span></td>
                                                    </tr>
                                                ";
                                    ...
                                    <?php
                                    for ($i = 0; $i < $count_app1; $i++){
                                        if($row=mysqli_fetch_assoc($result_app1)){
                                            $sid = $row['sid'];
                                            $sname = $row['sname'];
                                            $title = $row['title'];
                                            $jid = $row['jid'];
                                            ...
                                            if($status==1){
                                                ...
                                                echo "
                                                    <tr>
                                                        ...
                                                        <td><span class=\"label label-default\"> unreplied </span></td>
                                                        <td ><span id='open' class=\"label label-info open\">open</span><span  hidden>".$sid."</span><span  hidden>".$jid."</span></td>
                                                    </tr>
                                                ";
                                            }
                        ...
                        <div class="col-md-6 chart-layer1-right" id = "msg_right" style="display: none; width: 40%" >
                            <div class="user-marorm">
                                ...
                                <div class="malorm-bottom">
                                    ...
                                    <span id="comp_jid" hidden></span>
                                    <span id='accept' class="label label-success app">accept</span>
                                    <span id='reject' class="label label-danger app">reject</span>
